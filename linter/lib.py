...
class Session:
    ...
    def __init__(
        ...
        Attributes
        ----------
            json (dict | None): Optional dictionary to initialize the session with.
        """
        if json is None:
            json = {}
    ...
    def return_total(self):
        """
        Return the total number of tools in the current search cache.
        """
        return self._json["*"]["count"]

    def search_api(self, name: str, page: int = 1) -> None:
    ...
    def search_api_exact_match(self, name: str) -> None:
        """Retrieve JSON data from the biotools API.
        ...
        for page in range(page_start, page_end):
            ...
            try:
                ...
                if response.ok:
                    ...
                    self._json[f"{name}{page}"] = response.json()
                    continue
    ...
    def return_tool_list_json(self) -> list:
        ...
        """
        output = []
        for query in self._json.values():
            if "list" in query:
    ...
    def get_total_tool_count(self) -> int:
        ...
        return next(iter(self._json.items()))[1]["count"]
